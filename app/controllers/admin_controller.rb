class AdminController < ApplicationController
  ...
  def index
    ...
    @orders_by_day = @orders_by_day.group_by { |t| t.created_at.to_date }
    @revenue_by_day = @orders_by_day.map { |day,orders| [day.strftime("%A"), orders.sum(&:total)] }
    if @revenue_by_day.count < 7
      days_of_week = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']
      data_hash = Hash[@revenue_by_day]
      current_day = Time.now.strftime("%A")
      current_day_index = days_of_week.index(current_day)
      next_day_index = (current_day_index + 1) % days_of_week.count

      ordered_days_with_current_last = days_of_week[current_day_index..-1] + days_of_week[0...next_day_index]
